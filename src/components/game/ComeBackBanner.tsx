...
export const ComeBackBanner = ({ onClaimReward }: ComeBackBannerProps) => {
  ...
  useEffect(() => {
    ...
    const lastSessionKey = `last-session-${user.id}`;
    const claimedKey = `comeback-claimed-${user.id}`;
  ...
  const calculateReward = () => {
    // More days away = bigger reward (incentive to come back)
    const baseGems = 20;
    const bonusGems = Math.min(daysAway * 10, 50); // Max 50 bonus
    const lives = Math.min(Math.floor(daysAway / 2) + 2, 5); // 2-5 lives
  ...
  const handleClaim = () => {
    ...
    localStorage.setItem(`comeback-claimed-${user.id}`, new Date().toISOString().split('T')[0]);
