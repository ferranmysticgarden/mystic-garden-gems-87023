export interface Product {
  id: string;
  name: string;
  nameKey: string;
  price: number;
  currency: string;
  amount?: number;
  lives?: number | 'unlimited';
  gems?: number;
  powerups?: number;
  dailyGems?: number;
  noAdsDays?: number;
  noAdsForever?: boolean;
  instantGems?: number;
  allLevelsAccess?: boolean;
}

export const PRODUCTS: Product[] = [
  // STARTER GEMS - Welcome micro-offer €0.50
  {
    id: 'starter_gems',
    nameKey: 'shop.starterGems',
    name: '💎 Inicio Mágico',
    price: 0.50,
    currency: 'EUR',
    gems: 400,
  },
  { 
    id: 'quick_pack', 
    nameKey: 'shop.quickPack',
    name: '3 Vidas + 20 Gemas', 
    price: 0.99, 
    currency: 'EUR',
    lives: 3,
    gems: 20
  },
  { 
    id: 'gems_100', 
    nameKey: 'shop.gems100',
    name: '100 Gemas', 
    price: 0.99, 
    currency: 'EUR',
    amount: 100 
  },
  { 
    id: 'gems_300', 
    nameKey: 'shop.gems300',
    name: '300 Gemas', 
    price: 2.99, 
    currency: 'EUR',
    amount: 300 
  },
  { 
    id: 'gems_1200', 
    nameKey: 'shop.gems1200',
    name: '1200 Gemas', 
    price: 9.99, 
    currency: 'EUR',
    amount: 1200 
  },
  { 
    id: 'no_ads_month', 
    nameKey: 'shop.noAdsMonth',
    name: 'Quitar Anuncios (1 Mes)', 
    price: 1.99, 
    currency: 'EUR',
    noAdsDays: 30
  },
  { 
    id: 'no_ads_forever', 
    nameKey: 'shop.noAdsForever',
    name: 'Quitar Anuncios (Para Siempre)', 
    price: 4.99, 
    currency: 'EUR',
    noAdsForever: true
  },
  { 
    id: 'garden_pass', 
    nameKey: 'shop.gardenPass',
    name: 'Pase de Jardín Mensual', 
    price: 2.99, 
    currency: 'EUR',
    instantGems: 1000,
    noAdsDays: 30
  },
  // Flash Offer - 2 hour limited
  { 
    id: 'flash_offer', 
    nameKey: 'shop.flashOffer',
    name: 'Pack Relámpago', 
    price: 0.99, 
    currency: 'EUR',
    lives: 10,
    gems: 150,
    noAdsDays: 0 // 30 mins handled separately
  },
  // Victory Bonus
  { 
    id: 'victory_multiplier', 
    nameKey: 'shop.victoryMultiplier',
    name: 'Bono de Victoria (+2 Vidas)', 
    price: 0.50, 
    currency: 'EUR',
    lives: 2
  },
  // Ice Breaker - Termina este nivel (primera compra psicológica)
  { 
    id: 'finish_level', 
    nameKey: 'shop.finishLevel',
    name: 'Termina Este Nivel', 
    price: 0.50, 
    currency: 'EUR',
    powerups: 5 // +5 movimientos
  },
  // Almost had it - extra moves
  { 
    id: 'extra_moves', 
    nameKey: 'shop.extraMoves',
    name: 'Última Oportunidad', 
    price: 0.50, 
    currency: 'EUR',
    powerups: 5
  },
  // Chest Silver
  { 
    id: 'chest_silver', 
    nameKey: 'shop.chestSilver',
    name: 'Cofre Plata', 
    price: 1.99, 
    currency: 'EUR'
  },
  // Chest Gold
  { 
    id: 'chest_gold', 
    nameKey: 'shop.chestGold',
    name: 'Cofre Oro', 
    price: 3.99, 
    currency: 'EUR'
  },
  // First purchase pack
  { 
    id: 'first_purchase', 
    nameKey: 'shop.firstPurchase',
    name: 'Pack Primera Compra x5', 
    price: 0.50, 
    currency: 'EUR',
    gems: 500,
    lives: 20,
    noAdsDays: 1
  },
  // STARTER PACK - Irresistible offer for new players (after level 3-4)
  { 
    id: 'starter_pack', 
    nameKey: 'shop.starterPack',
    name: 'Starter Pack', 
    price: 1.99, 
    currency: 'EUR',
    gems: 500,
    lives: 10,
    powerups: 3
  },
  // CONTINUE GAME - Emotional offer when losing
  { 
    id: 'continue_game', 
    nameKey: 'shop.continueGame',
    name: 'Continuar Partida', 
    price: 0.50, 
    currency: 'EUR',
    lives: 1,
    powerups: 5 // +5 moves
  },
  // BUY MOVES - Before losing (0 moves) - Soft paywall €0.49
  {
    id: 'buy_moves',
    nameKey: 'shop.buyMoves',
    name: 'Comprar Movimientos',
    price: 0.50,
    currency: 'EUR',
    powerups: 5
  },
  // WELCOME PACK - First purchase offer after level 1
  {
    id: 'welcome_pack',
    nameKey: 'shop.welcomePack',
    name: 'Pack Bienvenida',
    price: 0.50,
    currency: 'EUR',
    powerups: 5, // +5 movimientos
    lives: 3 // +3 boosters (representado como vidas)
  },
  // REWARD DOUBLER - Post-victory €0.49
  {
    id: 'reward_doubler',
    nameKey: 'shop.rewardDoubler',
    name: 'Duplicar Recompensa x2',
    price: 0.50,
    currency: 'EUR'
  },
  // PACK VICTORIA SEGURA - Premium €2.99
  {
    id: 'pack_victoria_segura',
    nameKey: 'shop.packVictoriaSegura',
    name: 'Pack Victoria Segura',
    price: 2.99,
    currency: 'EUR',
    powerups: 5, // +5 movimientos
    lives: 3 // boosters / protección
  },
  // PACK RACHA INFINITA - Premium €1.99
  {
    id: 'pack_racha_infinita',
    nameKey: 'shop.packRachaInfinita',
    name: 'Pack Racha Infinita',
    price: 1.99,
    currency: 'EUR',
    lives: 2
    // + streak protection + 1 spin (handled by webhook)
  },
  // Pack Impulso €1.99
  {
    id: 'pack_impulso',
    nameKey: 'shop.packImpulso',
    name: 'Pack Impulso',
    price: 1.99,
    currency: 'EUR',
    powerups: 5, // +5 movimientos
    lives: 3 // +3 boosters
  },
  // Pack Experiencia €2.99
  {
    id: 'pack_experiencia',
    nameKey: 'shop.packExperiencia',
    name: 'Pack Experiencia',
    price: 2.99,
    currency: 'EUR',
    lives: 2, // 2 vidas extra
    // + protección racha + 1 giro (handled by webhook)
  },
  // Pack Victoria Segura Pro €3.99
  {
    id: 'pack_victoria_segura_pro',
    nameKey: 'shop.packVictoriaSeguraPro',
    name: 'Pack Victoria Segura Pro',
    price: 3.99,
    currency: 'EUR',
    powerups: 8, // +8 movimientos
    // + protección derrota + boosters premium (handled by webhook)
  },
  // MEGA PACK INICIAL - First Day Offer €2.99
  {
    id: 'mega_pack_inicial',
    nameKey: 'shop.megaPackInicial',
    name: 'Mega Pack de Bienvenida',
    price: 2.99,
    currency: 'EUR',
    gems: 500,
    lives: 10,
    powerups: 3,
    noAdsDays: 1,
  },
  // PACK REVANCHA - Lose Bundle
  {
    id: 'pack_revancha',
    nameKey: 'shop.packRevancha',
    name: 'Pack Revancha',
    price: 0.99,
    currency: 'EUR',
    gems: 50,
    lives: 5,
    powerups: 5,
  },
  // LIFESAVER PACK - When out of lives
  {
    id: 'lifesaver_pack',
    nameKey: 'shop.lifesaverPack',
    name: 'Pack Salvavidas',
    price: 0.50,
    currency: 'EUR',
    lives: 1,
    powerups: 3,
  },
  // STREAK PROTECTION
  {
    id: 'streak_protection',
    nameKey: 'shop.streakProtection',
    name: 'Protección de Racha',
    price: 0.50,
    currency: 'EUR',
  },
  // EXTRA SPIN - Lucky spin adicional
  {
    id: 'extra_spin',
    nameKey: 'shop.extraSpin',
    name: 'Giro Extra',
    price: 0.50,
    currency: 'EUR',
  },
  // VIDAS INFINITAS 30min - Trigger sin vidas
  {
    id: 'unlimited_lives_30min',
    nameKey: 'shop.unlimitedLives30',
    name: 'Vidas Infinitas 30min',
    price: 0.50,
    currency: 'EUR',
    lives: 'unlimited',
  },
  // ============ NUEVOS PRODUCTOS 2026-06 ============
  {
    id: 'piggy_bank_unlock',
    nameKey: 'shop.piggyBankUnlock',
    name: '🐷 Desbloquear Hucha',
    price: 2.99,
    currency: 'EUR',
  },
  {
    id: 'season_pass_premium',
    nameKey: 'shop.seasonPassPremium',
    name: '🏆 Pase de Temporada Premium',
    price: 4.99,
    currency: 'EUR',
  },
  {
    id: 'streak_bonus_5days',
    nameKey: 'shop.streakBonus5',
    name: '🔥 Bono Racha 5 Días',
    price: 3.99,
    currency: 'EUR',
    gems: 200,
    lives: 5,
  },
  {
    id: 'streak_bonus_7days',
    nameKey: 'shop.streakBonus7',
    name: '🔥 Bono Racha 7 Días',
    price: 5.99,
    currency: 'EUR',
    gems: 400,
    lives: 10,
    powerups: 3,
  },
  {
    id: 'streak_3wins_bonus',
    nameKey: 'shop.streak3Wins',
    name: '⚡ Bono 3 Victorias',
    price: 1.99,
    currency: 'EUR',
    gems: 100,
    lives: 3,
    powerups: 2,
  },
];
