  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (userId && productId) {
        ...
        // Calculate expiration for purchase record
        ...
        if (rewards && (rewards.gems || rewards.lives || rewards.powerups || rewards.noAdsDays || rewards.unlimitedLivesMinutes)) {
          ...
          // No-ads → no_ads_until (NOT unlimited_lives_until)
          if (rewards.noAdsDays) {
          ...
          if (rewards.unlimitedLivesMinutes) {
            ...
            const currentUL = progressData?.unlimited_lives_until
              ? new Date(progressData.unlimited_lives_until as string)
              : null;
          ...
          if (progressData) {
            ...
            if (rewards.noAdsDays) {
              insertData.no_ads_until = new Date(Date.now() + rewards.noAdsDays * 24 * 60 * 60 * 1000).toISOString();
            }
            if (rewards.unlimitedLivesMinutes) {
              insertData.unlimited_lives_until = new Date(Date.now() + rewards.unlimitedLivesMinutes * 60 * 1000).toISOString();
            }
